...
const AddKnowledgeModal = ({
  ...
  const validateUrl=(url:string)=>{
    try {
      ...
      return ["http:","https:"].includes(parsed.protocol); // protocol is https//..........
    } catch (error) {
      return false;
    }
  }
  ...
  const handleImportWrapper= async()=>{
    ...
    if(defaultTab==="website"){
      ...
      if(!validateUrl(websiteUrl)){
        setError("Please enter the valid url (e.g. https//example.com)");
        return;
      }
      ...
      if(exists){
        setError("this website is already in your knowlegde base.")
        return;
      }
      data.url=websiteUrl;
        ...
        >
          ...
          <div className="p-6 min-h-50 space-y-4">
            ...
            >
              ...
              />
              <div
                className="border-2 border-dashed border-white/10 rounded-xl h-60 flex flex-col items-center justify-center gap-0.5 bg-zinc-700/30"
                onClick={() => {
                  document.getElementById("csv-file-input")?.click();
                }}
              >
                <div className="w-12 h-12 rounded-full bg-zinc-800 flex items-center justify-center cursor-pointer border border-amber-200 hover:border-black">
          ...
          <div className="p-6 border-t border-white/5 flex bg-black/20 justify-end  gap-3">
            <Button
             variant="ghost" onClick={()=>setIsOpen(false)} className="text-zinc-400 hover:text-white hover:bg-white/5"
            >Cancel</Button>
